...
namespace sdds
{
    ...
    void CustomerOrder::fillItem(Station &station, std::ostream &os)
    {
        ...
        if (contains)
        {
            ...
            if (station.getQuantity() >= 1 && index > -1)
            {
    ...
    void CustomerOrder::display(std::ostream &os) const
    {
        ...
        for(size_t i=0;i<m_cntItem;i++)
        {
            os<<"["<<setw(6)<<setfill('0')<<m_lstItem[i]->m_serialNumber<<"] "<<setfill(' ')<<setw(CustomerOrder::m_widthField)<<left<<m_lstItem[i]->m_itemName<<"   - "<<(m_lstItem[i]->m_isFilled?"FILLED" : "TO BE FILLED")<<endl;
        }
